<template>
  <nav class="relative bg-white w-full pt-4 flex justify-between">
    ...
    <button class="block md:hidden text-4xl mr-8 -mt-2" @click="openNav">
      <ClientOnly><font-awesome-icon icon="bars" /></ClientOnly>
    </button>
    <div
      class="absolute md:hidden top-0 flex flex-col h-screen w-full z-[100] bg-white transition-all delay-75 duration-300"
      :class="isNavOpen ? ' right-0 md:-right-full' : '-right-full'"
    >
      <button class="text-4xl self-end mr-8 mt-8 mb-6 p-1" @click="closeNav">
        <ClientOnly>
